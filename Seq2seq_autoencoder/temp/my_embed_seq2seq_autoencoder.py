...
from data_utils import load_and_preprocess_data, load_embeddings, read_txt_data, ModelHelper
import my_rnn_cell
import my_rnn
...
# in model.py. Therefore, it can use self.build() for e.g.
    ...
    def encode_on_batch(self, sess, inputs_batch, labels_batch, mask_batch):
        ...
        encodings = sess.run(self.add_encoding_op(), feed_dict=feed)
        return encodings

    def encode(self, sess, inputs_raw, inputs=None):
        if inputs is None:
...
def test_encoding(args):
    ...
    print "number of tokens:", len(helper.tok2id)
    input_data = read_txt_data(args.data)
