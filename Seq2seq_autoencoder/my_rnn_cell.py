...
from tensorflow.python.ops.gen_nn_ops import relu #BK
...
class BasicRNNCell(RNNCell):
  ...
  def __init__(self, num_units, cell_init="random", input_size=None, activation=tanh):
    if input_size is not None:
      logging.warn("%s: The input_size parameter is deprecated.", self)
    self._num_units = num_units
    self._activation = activation
    self._cell_init = cell_init
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    if self._cell_init == "random":
      with vs.variable_scope(scope or type(self).__name__):  # "BasicRNNCell"
        output = self._activation(_linear([inputs, state], self._num_units, True))
    elif self._cell_init == "identity":
      with vs.variable_scope(scope or type(self).__name__):
        self._activation = relu
        output = self._activation(_my_linear([inputs, state], self._num_units, self._cell_init, True))
    else:
      raise ValueError("cell_init has to be either random or identity.")
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with vs.variable_scope(scope or type(self).__name__):  # "GRUCell"
      with vs.variable_scope("Gates"):  # Reset gate and update gate.
        ...
        r, u = array_ops.split(1, 2, _my_linear([inputs, state],
                                             2 * self._num_units, self._cell_init, True, 1.0))
        r, u = sigmoid(r), sigmoid(u)
      with vs.variable_scope("Candidate"):
        c = self._activation(_my_linear([inputs, r * state],
                                     self._num_units, self._cell_init, True))
      new_h = u * state + (1 - u) * c
    return new_h, new_h
...
class BasicLSTMCell(RNNCell):
  ...
  def __init__(self, num_units, cell_init="random", forget_bias=1.0, input_size=None,
  ...
  def __call__(self, inputs, state, scope=None):
    ...
    with vs.variable_scope(scope or type(self).__name__):  # "BasicLSTMCell"
      ...
      concat = _my_linear([inputs, h], 4 * self._num_units, self._cell_init, True)
...
def _my_linear(args, output_size, cell_init="random", bias=True, bias_start=0.0, scope=None):
  ...
  r = int(output_size/arg_sizes[1]) #Even though both numerator and denominator are integers, their ratio is float type. So, we need to cast it to int.
  ...
  with vs.variable_scope(scope or "Linear"): 
    if cell_init == "random":
      ...
      with vs.variable_scope(scope or "identity"):
        ...
        concat_id = np.concatenate(list_id, 1) # (arg_sizes[1], output_size)

        epsilon = np.sqrt(6.0/(arg_sizes[0]+output_size))
        xavier_part = np.random.uniform(-epsilon, epsilon, (arg_sizes[0], output_size))

        total_init_matrix = np.concatenate([xavier_part, concat_id],0) # (arg_sizes[0]+arg_sizes[1], output_size)
        init = tf.constant_initializer(total_init_matrix)

        my_matrix = vs.get_variable("my_matrix", [total_arg_size, output_size], initializer=init, dtype=dtype)
        if len(args) == 1:
