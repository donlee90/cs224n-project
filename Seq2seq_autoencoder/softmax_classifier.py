...
class SoftmaxConfig(object): #For my own benefit: (object) means that Config class inherits from "object" class, and in practice I can replace it with ().
...
class SoftmaxModel(Model): #For my own benefit: (Model) means SoftmaxModel class inherits from Model class defined in model.py. Therefore, it can use self.build() for e.g.
    ...
    def fit(self, sess, train, dev, test):
        ...
        Args:
            sess: tf.Session()
            train:  list of [encodings, labels]
            dev:    list of [encodings, labels]
            test:   list of [encodings, labels]
        Returns:
